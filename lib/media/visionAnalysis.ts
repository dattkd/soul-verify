...
/**
 * Analyze a text string (caption, article, post body) for AI-generated content.
 * Used as a fallback when no media can be downloaded from a URL.
 */
export async function analyzeTextContent(text: string): Promise<VisionAnalysisResult | null> {
  if (!text?.trim()) return null;
  const client = new Anthropic();
  const prompt = `Analyze the following text and determine the probability (0-100) that it was generated by an AI language model rather than written by a human.

Look for:
- Unnaturally smooth, error-free prose with no personal voice
- Vague, generic, or non-specific descriptions that could apply to anyone
- Repetitive sentence structures or overuse of transitional phrases
- Marketing-speak or "engagement-bait" phrasing ("dive into", "it's important to note")
- Lack of genuine emotion, quirks, typos, or human imperfection
- Suspiciously comprehensive or well-rounded coverage of a topic
- Inconsistent first-person voice or mixed perspectives
- Generic hashtags or SEO-style keyword stuffing
...
SCORE: [0-100]
SIGNALS: [comma-separated list of observed signals, max 5]
REASONING: [one sentence]`;
  ...
  try {
    ...
    if (result) {
      console.log(`[vision] Text analysis: ${result.aiProbability}% AI probability`);
    }
    return result;
  } catch (err) {
    console.error('[vision] Text analysis error:', err instanceof Error ? err.message : err);
    return null;
  }
}
